...
def Guess():
    try:
        ...
        while computer != secret:
          computer = random.randint(0, a)
        endTime = time.time()
        print(f"The computer successfully guessed your number. It guessed {str(computer)} and your number was {str(secret)}. The time taken for the computer to get this result was {startTime - endTime} seconds!")
        again = input("Do you want to play the game again? Y/N: ").lower()
        while again not in k:
